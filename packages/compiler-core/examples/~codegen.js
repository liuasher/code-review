...
function _generate(ast, options){
  return {
    ast,
    code: context.code,
 ...
 * 方法出参：ast、code
 * 方法总结：
 *    1、针对NodeTypes.FOR，能返回序列表达式参数
 */
const untest012 = generate(
  createRoot({
    codegenNode: {
      type: 11,
      loc: locStub,
 ...
 * 方法出参：ast、code
 * 方法总结：
 *    0、createElementWithCodegen 根据Codegen创建对象
 *    1、createSimpleExpression   创建一个声明字符串的表达式
 *    2、createObjectExpression   创建一个对象
 *    3、createObjectProperty     创建一个对象的属性
 */
const untest013 = generate(
  createRoot({
 ...
 * @param { Number 014 }
 * @function generate 
 * 方法入参：createArrayExpression 创建数组表达式
 * 方法出参：ast、code
 * 方法总结：
 *    1、能返回正确的数组表达式
 */
const untest014 = generate(
  createRoot({
 ...
 * @param { Number 015 }
 * @function generate 
 * 方法入参：createSequenceExpression 创建序列表达式
 * 方法出参：ast、code
 * 方法总结：
 *    1、能返回正确的序列表达式
 */
const untest015 =  generate(
  createRoot({
    codegenNode: createSequenceExpression([
      createSimpleExpression(`foo`, false),
      createCallExpression(`bar`, [`baz`])
 ...
 * @param { Number 016 }
 * @function generate 
 * 方法入参：createConditionalExpression 创建条件表达式
 * 方法出参：ast、code
 * 方法总结：
 *    1、能返回正确的条件表达式
 */
const untest016 = generate(
  createRoot({
 ...
 * @param { Number 017 }
 * @function generate 
 * 方法入参：createCacheExpression 创建缓存表达式
 * 方法出参：ast、code
 * 方法总结：
 *    1、能返回正确的缓存表达式
 */
const untest017 = generate(
  createRoot({
    cached: 1,
 ...
 * @param { Number 018 }
 * @function generate 
 * 方法入参：createCacheExpression 创建缓存表达式，多了一个true
 * 方法出参：ast、code
 * 方法总结：
 *    1、能返回正确的缓存表达式
 *    2、多了一个setBlockTracking
 */
const untest018 = generate(
  createRoot({
    cached: 1,
    codegenNode: createCacheExpression(
      1,
