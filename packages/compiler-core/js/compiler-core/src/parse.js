...
function parseChildren(context, mode, ancestors) {
    ...
    while (!isEnd(context, mode, ancestors)) {
        true && utils_1.assert(context.source.length > 0);
...
function parseCDATA(context, ancestors) {
    ...
    true && utils_1.assert(startsWith(context.source, '<![CDATA['));
    advanceBy(context, 9);
    ...
    if (context.source.length === 0) {
        ...
        true && utils_1.assert(startsWith(context.source, ']]>'));
        advanceBy(context, 3);
    }
    return nodes;
}
...
function parseBogusComment(context) {
    true && utils_1.assert(/^<(?:[\!\?]|\/[^a-z>])/i.test(context.source));
    var start = getCursor(context);
...
function parseInterpolation(context, mode) {
    ...
    true && utils_1.assert(startsWith(context.source, open));
...
function parseText(context, mode) {
    ...
    true && utils_1.assert(endIndex > 0);
    var start = getCursor(context);
...
function advanceBy(context, numberOfCharacters) {
    ...
    true && utils_1.assert(numberOfCharacters <= source.length);
